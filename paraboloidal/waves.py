...
X_eval = laguerreX(m, n, r, no_factorials=True)
P_eval = laguerreP(m, n, r, method='known_functions').astype(complex)
L_eval = laguerreL(n, m, r).flatten()
...
P_large_n = laguerreP(m, n, r).astype(complex)
...
X_eval = laguerreX(m, n, r).astype(complex).flatten()

def laguerre_eq(m, n, z, kind='pinney'):
    # lambda to evaluate function
    if kind == 'laguerre':
        evaluate_U = lambda z: laguerreL(n, m, z)
    elif kind == 'X':
        evaluate_U = lambda z: laguerreX(m, n, z)
    elif kind == 'pinney':
        evaluate_U = lambda z: laguerreP(m, n, z)
    else:
